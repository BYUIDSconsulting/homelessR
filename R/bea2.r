...
agg_state_year <- function(data) {
  return(data |>
           ...
           dplyr::select(data$TimePeriod, data$DataValue, data$state) |>
           dplyr::group_by(data$state, data$TimePeriod) |>
           dplyr::mutate(DataValue = sum(data$DataValue)) |>
           dplyr::rename(Year = data$TimePeriod) |>
           dplyr::distinct()
  )
}
